...
import os
import importlib
#eval = importlib.machinery.SourceFileLoader('eval','eval/run_eval.py').load_module()
...
def generateRes(model):
    ...
    for i in range(0,5):
        ...
        query = "terrier/bin/terrier batchretrieve -q -w " + model + " -t " + data
        os.system(query) #获得训练的res文件
        data_test = "terrier/collection/test" + str(i) + ".xml"
        query = "terrier/bin/terrier batchretrieve -q -w " + model + " -t" + data_test
...
def generateEvalute(model,file_number):
    ...
    for i in range(file_number-10,file_number,2):
        train = trec_evaluate(golden_file,result_path+model+"_d_3_t_10_" + str(i) + ".res")
        score_train.append(train)
        test = trec_evaluate(golden_file,result_path+model+"_d_3_t_10_" + str(i+1) + ".res")
...
def baseModel():
    ...
    for i in model:
        ...
        model_result.append(
            [i, "extension", #sum(score_train[:, 0]) / 5, sum(score_train[:, 1]) / 5, sum(score_train[:, 2]) / 5,
